...
export class UsersController {
  ...
  createUser(
    @Body() { firstName, lastName, email, phone, password }: createUserDto,
  ) {
  ...
  @Post('verify-pin')
  verifyPin(
    @DecodedTokenDecorator() id: string,
    @Body()
    createPin: createPin,
  ) {
    return this.userService.verifyTransactionPin(id, createPin.pin);
  }
  ...
  updateAddress(
    @Param('id') id: string,
    @Body() { address, city, state, lga }: any,
  ) {
  ...
  updatePassword(
    @Param('id') id: string,
    @Body() { password, new_password }: any,
  ) {
  ...
  updateUserAccount(@Body() { lastName, firstName, phone, id }: UpdateUserDto) {
    return this.userService.updateUserAccount({
      lastName,
      firstName,
      phone,
      id,
  ...
  verifyUserBvn(@DecodedTokenDecorator() id: string, @Body() { bvn }: any) {
